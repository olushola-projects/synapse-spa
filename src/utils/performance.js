...
export const lazyLoad = (importFunc, fallback) => {
    ...
    return (props) => (_jsx(React.Suspense, { fallback: fallback ? React.createElement(fallback) : _jsx("div", { children: "Loading..." }), children: _jsx(LazyComponent, { ...props }) }));
};
/**
...
export const OptimizedImage = ({ src, alt, className = '', width, height }) => {
    ...
    return (_jsxs("picture", { className: className, children: [_jsx("source", { srcSet: webpSrc, type: 'image/webp' }), _jsx("img", { src: src, alt: alt, width: width, height: height, loading: 'lazy', decoding: 'async' })] }));
};
/**
...
export const usePerformanceMonitor = (componentName) => {
    ...
    const trackApiCall = useCallback(async (apiCall, apiName) => {
...
export class PerformanceAnalytics {
     ...
     */
    recordMetrics(componentName, metrics) {
        if (!this.metrics.has(componentName)) {
            this.metrics.set(componentName, []);
        }
        const componentMetrics = this.metrics.get(componentName);
        componentMetrics.push({ ...metrics, timestamp: Date.now() });
     ...
     */
    getPerformanceSummary(componentName) {
        const componentMetrics = this.metrics.get(componentName) || [];
