...
class PerformanceMonitor {
    ...
    updateMetrics() {
        ...
        const recentClassifications = this.classificationHistory.filter(record => record.timestamp > oneHourAgo);
    ...
    getSystemHealthMetrics() {
        ...
        const recentHealthChecks = this.metrics.filter(m => m.metric === 'health_check_status' && m.timestamp > oneHourAgo);
        const healthCheckSuccesses = recentHealthChecks.filter(m => m.value === 1).length;
        const uptimePercentage = recentHealthChecks.length > 0 ? (healthCheckSuccesses / recentHealthChecks.length) * 100 : 0;
