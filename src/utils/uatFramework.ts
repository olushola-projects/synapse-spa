...
export interface UATTestCase {
  ...
  name: string;
  description: string;
  category: 'functional' | 'usability' | 'performance' | 'integration' | 'compliance';
  priority: 'low' | 'medium' | 'high' | 'critical';
  stakeholder: 'grc_professional' | 'compliance_manager' | 'end_user' | 'administrator';
...
export interface UATTestSuite {
  ...
  executionPlan: UATExecutionPlan;
  results: UATSuiteResults;
}

export interface UATExecutionPlan {
  plannedStartDate: string;
  plannedEndDate: string;
  executors: UATExecutor[];
  schedule: UATScheduleItem[];
  dependencies: string[];
}

export interface UATExecutor {
  name: string;
  role: string;
  email: string;
...
export interface UATScheduleItem {
  date: string;
  testCaseIds: string[];
  executor: string;
...
export interface UATIssue {
  ...
  category: 'functional' | 'ui' | 'performance' | 'data' | 'integration';
  description: string;
...
export class UATFramework {
  private testSuites: Map<string, UATTestSuite> = new Map();
  private issues: Map<string, UATIssue> = new Map();
   ...
   */
  private createSFDRClassificationTestSuite(): UATTestSuite {
    const testCases: UATTestCase[] = [
      {
        id: 'SFDR_001',
   ...
   */
  private createNexusAgentTestSuite(): UATTestSuite {
    const testCases: UATTestCase[] = [
      {
        id: 'NEXUS_001',
   ...
   */
  private createUserExperienceTestSuite(): UATTestSuite {
    const testCases: UATTestCase[] = [
      {
        id: 'UX_001',
   ...
   */
  private createPerformanceTestSuite(): UATTestSuite {
    const testCases: UATTestCase[] = [
      {
        id: 'PERF_001',
   ...
   */
  private createIntegrationTestSuite(): UATTestSuite {
    const testCases: UATTestCase[] = [
      {
        id: 'INT_001',
   ...
   */
  async executeTestCase(suiteId: string, testCaseId: string): Promise<UATTestCase> {
   ...
   */
  private async executeFunctionalTest(testCase: UATTestCase): Promise<void> {
   ...
   */
  private async executeIntegrationTest(testCase: UATTestCase): Promise<void> {
    for (const step of testCase.testSteps) {
   ...
   */
  private updateSuiteResults(suiteId: string): void {
    const suite = this.testSuites.get(suiteId);
    if (!suite) return;
   ...
   */
  getTestSuite(suiteId: string): UATTestSuite | undefined {
