...
describe('ErrorBoundary', () => {
    ...
    it('should call error handler when error occurs', () => {
        render(_jsx(ErrorBoundary, { children: _jsx(ThrowError, { shouldThrow: true }) }));
        expect(mockErrorHandler.handleError).toHaveBeenCalledWith(expect.any(Error), 'system', 'critical', expect.objectContaining({
            component: 'ErrorBoundary',
            errorBoundary: true
        }));
    });
    it('should reset error state when Try Again is clicked', () => {
        const { rerender } = render(_jsx(ErrorBoundary, { children: _jsx(ThrowError, { shouldThrow: true }) }));
    ...
    it('should display custom fallback when provided', () => {
        ...
        render(_jsx(ErrorBoundary, { fallback: _jsx(CustomFallback, {}), children: _jsx(ThrowError, { shouldThrow: true }) }));
    ...
    it('should preserve error information in error handler call', () => {
        ...
        expect(mockErrorHandler.handleError).toHaveBeenCalledWith(testError, 'system', 'critical', expect.objectContaining({
            component: 'ErrorBoundary',
            errorBoundary: true,
    ...
    it('should not interfere with normal React lifecycle', () => {
        const TestComponent = () => {
            ...
            return (_jsxs("div", { children: [_jsxs("span", { children: ["Count: ", count] }), _jsx("button", { onClick: () => setCount(c => c + 1), children: "Increment" })] }));
        };
