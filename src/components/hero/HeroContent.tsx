...
export const HeroContent: React.FC<HeroContentProps> = ({ animate, onGetAccess, onInvite }) => {
  return (
    <div className={`w-full text-left transition-all duration-700 ease-out pl-4 md:pl-6 lg:pl-8 pr-4 md:pr-6 lg:pr-8 pb-10 md:pb-0 ${animate ? 'opacity-100' : 'opacity-0 translate-y-6'}`}>
      {/* Main Headline - Optimized font sizing for better readability */}
      <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight leading-tight text-blue-600 mb-6" 
          style={{
      ...
      <p className="mt-6 text-sm md:text-base lg:text-lg text-slate-600 max-w-2xl leading-relaxed font-normal">
        Join a global network of GRC professionals<br />
        leveraging AI agents for compliance excellence.<br />
        Access exclusive testing of future solutions,<br />
        comprehensive regulatory insights, and personalized<br />
        career resilience tools to <span className="font-semibold text-blue-600">upskill, adapt, and lead</span> the<br />
      ...
      <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-start items-start">
        <div onClick={onGetAccess} className="cursor-pointer">
          <Button className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 text-base rounded-xl flex items-center gap-3 hover-lift font-semibold shadow-lg hover:shadow-xl transition-all duration-300">
            Get Early Access <ArrowRight size={20} className="transition-transform group-hover:translate-x-1" />
          </Button>
        </div>
