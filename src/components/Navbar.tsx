        ...
        <div className='w-full max-w-7xl mx-auto'>
          <div className='flex items-center justify-between h-16 pl-4 md:pl-6 lg:pl-8 pr-4 md:pr-6 lg:pr-8'>
            ...
            delay: 0.1
              ...
              <Link to='/' className='text-2xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent hover:from-accent hover:to-primary transition-all duration-300'>
                Synapses
              </Link>
            </motion.div>
            ...
            <nav className='hidden lg:flex items-center space-x-8'>
              {/* Agents Dropdown */}
              <div className='relative' onMouseEnter={() => setAgentsDropdownOpen(true)} onMouseLeave={() => setAgentsDropdownOpen(false)}>
                ...
                <AnimatePresence>
                  {agentsDropdownOpen && <motion.div initial={{
                  opacity: 0,
                  y: 10,
                  scale: 0.95
                }} animate={{
                  ...
                  ease: [0.23, 1, 0.32, 1]
                      ...
                      <div className='p-4'>
                        <Link to='/cdd-agent' className='flex items-start space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200 group'>
                        ...
                        <Link to='/nexus-agent' className='flex items-start space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200 group'>
              ...
              {/* Use Cases */}
              <Link to='/use-cases' className='text-foreground/80 hover:text-foreground font-medium transition-colors duration-200'>
                Use Cases
              </Link>

              {/* Partners */}
              <Link to='/partners' className='text-foreground/80 hover:text-foreground font-medium transition-colors duration-200'>
                Partners
              </Link>
            </nav>
            ...
            <div className='hidden lg:flex items-center space-x-4'>
              <Button variant='outline' size='sm' onClick={openInviteDialog} className='border-border/60 text-foreground hover:bg-muted/50 font-medium px-6 rounded-lg transition-all duration-200'>
                <Users className='h-4 w-4 mr-2' />
                Invite
              </Button>

              <Button size='sm' onClick={openFormDialog} className='bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-white font-semibold px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-200'>
                Get Early Access
              </Button>
            </div>
          ...
          ease: [0.23, 1, 0.32, 1]
              ...
              <div className='w-full max-w-7xl mx-auto pl-4 md:pl-6 lg:pl-8 pr-4 md:pr-6 lg:pr-8 py-6 space-y-4'>
                ...
                <div className='space-y-3'>
                  ...
                  <Link to='/cdd-agent' onClick={closeMobileMenu} className='flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200'>
                    <Shield className='h-5 w-5 text-primary' />
                    <span className='font-medium text-foreground'>CDD Agent</span>
                  </Link>
                  <Link to='/nexus-agent' onClick={closeMobileMenu} className='flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200'>
                    <img src="/lovable-uploads/794c2751-9650-4079-ab13-82bacd5914db.png" alt="Sophia SFDR Navigator" className="w-5 h-5 rounded-full object-cover" />
                ...
                <div className='space-y-3 pt-4 border-t border-border/20'>
                  <Link to='/use-cases' onClick={closeMobileMenu} className='flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200'>
                    <Briefcase className='h-5 w-5 text-foreground/60' />
                    <span className='font-medium text-foreground'>Use Cases</span>
                  </Link>
                  <Link to='/partners' onClick={closeMobileMenu} className='flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors duration-200'>
