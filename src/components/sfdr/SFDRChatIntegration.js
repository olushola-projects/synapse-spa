...
const SFDRChatIntegration = () => {
    ...
    const formatChatResponse = (classification) => {
        ...
        const explainabilityScore = classification.explainability_score
            ...
            : '';
        const regulatoryBasis = classification.regulatory_basis && classification.regulatory_basis.length > 0
        ...
        const benchmarkInfo = classification.benchmark_comparison
            ...
            : '';
        const keyIndicators = classification.key_indicators && classification.key_indicators.length > 0
            ? `\n🎯 **Key ESG Indicators**: ${classification.key_indicators.join(', ')}\n`
            : '';
        const riskFactors = classification.risk_factors && classification.risk_factors.length > 0
...
${auditInfo}${processingTime}`;
    ...
    return (_jsxs(Card, { className: 'w-full max-w-4xl mx-auto', children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: 'flex items-center gap-2', children: [_jsx(History, { className: 'h-5 w-5' }), "SFDR Chat Integration", _jsxs(Badge, { variant: apiHealth === 'healthy' ? 'default' : 'destructive', children: [apiHealth === 'checking' && _jsx(Loader2, { className: 'h-3 w-3 animate-spin mr-1' }), apiHealth === 'healthy' && _jsx(CheckCircle, { className: 'h-3 w-3 mr-1' }), apiHealth === 'error' && _jsx(AlertTriangle, { className: 'h-3 w-3 mr-1' }), apiHealth === 'checking'
                                    ? 'Checking...'
                                    : apiHealth === 'healthy'
                                        ? 'Connected'
                                        : 'Disconnected'] })] }) }), _jsxs(CardContent, { className: 'space-y-4', children: [_jsxs("div", { className: 'min-h-[400px] max-h-[400px] overflow-y-auto border rounded-lg p-4 space-y-4', children: [messages.length === 0 && (_jsxs("div", { className: 'text-center text-muted-foreground', children: [_jsx(MessageSquare, { className: 'h-12 w-12 mx-auto mb-2 opacity-50' }), _jsx("p", { children: "Start a conversation about SFDR compliance..." }), _jsx("p", { className: 'text-sm', children: "Try asking about \"Article 8 fund classification\" or \"Green bond compliance\"" })] })), messages.map(message => (_jsx("div", { className: `flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`, children: _jsxs("div", { className: `max-w-[80%] rounded-lg p-3 ${message.sender === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`, children: [_jsx("pre", { className: 'whitespace-pre-wrap font-sans text-sm', children: message.content }), _jsx("div", { className: 'text-xs opacity-70 mt-1', children: message.timestamp.toLocaleTimeString() })] }) }, message.id))), loading && (_jsx("div", { className: 'flex justify-start', children: _jsxs("div", { className: 'bg-muted rounded-lg p-3 flex items-center gap-2', children: [_jsx(Loader2, { className: 'h-4 w-4 animate-spin' }), _jsx("span", { className: 'text-sm', children: "Analyzing SFDR compliance..." })] }) }))] }), _jsxs("div", { className: 'flex gap-2', children: [_jsx(Input, { value: input, onChange: e => setInput(e.target.value), onKeyPress: handleKeyPress, placeholder: 'Ask about SFDR compliance, fund classification, or regulations...', disabled: loading || apiHealth === 'error' }), _jsx(Button, { onClick: sendMessage, disabled: !input.trim() || loading || apiHealth === 'error', children: loading ? _jsx(Loader2, { className: 'h-4 w-4 animate-spin' }) : 'Send' })] }), apiHealth === 'error' && (_jsx("div", { className: 'text-sm text-destructive bg-destructive/10 p-2 rounded', children: "\u26A0\uFE0F SFDR API is currently unavailable. Please check your configuration or try again later." }))] })] }));
};
