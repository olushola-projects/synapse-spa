...
export const EnterpriseUATAuditDashboard = () => {
    ...
    return (_jsxs("div", { className: 'space-y-6', children: [_jsxs(Card, { children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { className: 'flex items-center gap-2', children: [_jsx(Shield, { className: 'h-5 w-5' }), "Enterprise UAT & Security Audit Dashboard"] }) }), _jsxs(CardContent, { children: [_jsxs("div", { className: 'grid grid-cols-1 md:grid-cols-5 gap-4 mb-6', children: [_jsxs("div", { className: 'text-center', children: [_jsxs("div", { className: 'text-2xl font-bold text-success', children: [metrics.complianceScore, "%"] }), _jsx("div", { className: 'text-sm text-muted-foreground', children: "Compliance Score" })] }), _jsxs("div", { className: 'text-center', children: [_jsxs("div", { className: 'text-2xl font-bold text-primary', children: [metrics.riskScore, "%"] }), _jsx("div", { className: 'text-sm text-muted-foreground', children: "Risk Score" })] }), _jsxs("div", { className: 'text-center', children: [_jsx("div", { className: 'text-2xl font-bold text-muted-foreground', children: metrics.totalIssues }), _jsx("div", { className: 'text-sm text-muted-foreground', children: "Total Issues" })] }), _jsxs("div", { className: 'text-center', children: [_jsx("div", { className: 'text-2xl font-bold text-destructive', children: metrics.criticalIssues }), _jsx("div", { className: 'text-sm text-muted-foreground', children: "Critical Issues" })] }), _jsxs("div", { className: 'text-center', children: [_jsx("div", { className: 'text-2xl font-bold text-warning', children: metrics.highRiskIssues }), _jsx("div", { className: 'text-sm text-muted-foreground', children: "High Risk Issues" })] })] }), _jsx(Button, { onClick: runComprehensiveAudit, disabled: isRunning, className: 'w-full', size: 'lg', children: isRunning ? 'Running Enterprise Audit...' : 'Run Comprehensive Big 4 / RegTech Audit' })] })] }), metrics.criticalIssues > 0 && (_jsxs(Alert, { className: 'border-destructive', children: [_jsx(AlertTriangle, { className: 'h-4 w-4' }), _jsxs(AlertDescription, { children: [_jsx("strong", { children: "CRITICAL ISSUES DETECTED:" }), " ", metrics.criticalIssues, " critical issues require immediate remediation before production deployment."] })] })), _jsxs(Tabs, { defaultValue: 'security', className: 'space-y-4', children: [_jsxs(TabsList, { className: 'grid w-full grid-cols-6', children: [_jsx(TabsTrigger, { value: 'security', children: "Security" }), _jsx(TabsTrigger, { value: 'api', children: "API Integration" }), _jsx(TabsTrigger, { value: 'data', children: "Data Governance" }), _jsx(TabsTrigger, { value: 'regulatory', children: "Regulatory" }), _jsx(TabsTrigger, { value: 'continuity', children: "Continuity" }), _jsx(TabsTrigger, { value: 'performance', children: "Performance" })] }), ['security', 'api', 'data', 'regulatory', 'continuity', 'performance'].map(category => (_jsx(TabsContent, { value: category, className: 'space-y-4', children: auditResults
                            .filter(result => result.category
                            .toLowerCase()
                            .includes(category === 'security'
                            ? 'security'
                            : category === 'api'
                                ? 'integration'
                                : category === 'data'
                                    ...
                                    : category === 'regulatory'
                                        ...
                                        : category === 'continuity'
                                            ? 'continuity'
                                            : 'performance'))
                            .map((result, index) => (_jsx(motion.div, { initial: { opacity: 0, y: 10 }, animate: { opacity: 1, y: 0 }, transition: { delay: index * 0.1 }, children: _jsx(Card, { className: `border-l-4 ${result.status === 'pass'
                                    ? 'border-l-success'
                                    : result.status === 'fail'
                                        ? 'border-l-destructive'
                                        : result.status === 'warning'
                                            ? 'border-l-warning'
                                            : 'border-l-muted'}`, children: _jsxs(CardContent, { className: 'pt-4', children: [_jsxs("div", { className: 'flex items-center justify-between mb-2', children: [_jsxs("div", { className: 'flex items-center gap-2', children: [getStatusIcon(result.status), _jsx("h4", { className: 'font-semibold', children: result.test })] }), getRiskBadge(result.risk)] }), _jsx("p", { className: 'text-sm text-muted-foreground mb-2', children: result.finding }), result.remediation && (_jsx(Alert, { className: 'mt-2', children: _jsxs(AlertDescription, { className: 'text-xs', children: [_jsx("strong", { children: "Remediation Required:" }), " ", result.remediation] }) })), _jsx("div", { className: 'flex flex-wrap gap-1 mt-2', children: result.compliance.map(comp => (_jsx(Badge, { variant: 'outline', className: 'text-xs', children: comp }, comp))) })] }) }) }, index))) }, category)))] })] }));
};
