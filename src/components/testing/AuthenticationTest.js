...
export function AuthenticationTest() {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'success':
                return (_jsx(Badge, { variant: 'default', className: 'bg-green-100 text-green-800', children: "Passed" }));
            case 'error':
    ...
    return (_jsxs(Card, { className: 'w-full max-w-4xl mx-auto', children: [_jsxs(CardHeader, { children: [_jsxs(CardTitle, { className: 'flex items-center gap-2', children: [_jsx(Shield, { className: 'h-5 w-5' }), "Authentication & Security Test Suite"] }), _jsx(CardDescription, { children: "Comprehensive testing of authentication flow, edge function proxy, and API connectivity" })] }), _jsxs(CardContent, { className: 'space-y-4', children: [_jsx(Alert, { children: _jsx(AlertDescription, { children: "\uD83D\uDD10 This test suite verifies that the security remediation plan is working correctly. All API calls go through the secure edge function proxy with proper authentication." }) }), _jsx(Button, { onClick: runTests, disabled: isRunning, className: 'w-full', children: isRunning ? 'Running Tests...' : 'Run Authentication Tests' }), tests.length > 0 && (_jsxs("div", { className: 'space-y-3', children: [_jsx("h3", { className: 'text-lg font-semibold', children: "Test Results" }), tests.map((test, index) => (_jsxs(Card, { className: 'p-4', children: [_jsxs("div", { className: 'flex items-center justify-between mb-2', children: [_jsxs("div", { className: 'flex items-center gap-2', children: [getStatusIcon(test.status), _jsx("span", { className: 'font-medium', children: test.name })] }), _jsxs("div", { className: 'flex items-center gap-2', children: [test.duration && (_jsxs("span", { className: 'text-sm text-muted-foreground', children: [test.duration, "ms"] })), getStatusBadge(test.status)] })] }), _jsx("p", { className: 'text-sm text-muted-foreground mb-2', children: test.message }), test.details && (_jsxs("details", { className: 'text-xs', children: [_jsx("summary", { className: 'cursor-pointer text-muted-foreground hover:text-foreground', children: "View Details" }), _jsx("pre", { className: 'mt-2 p-2 bg-muted rounded overflow-auto', children: JSON.stringify(test.details, null, 2) })] }))] }, index)))] })), tests.length > 0 && !isRunning && (_jsx(Alert, { children: _jsx(AlertDescription, { children: tests.every(t => t.status === 'success')
