...
const TestReportDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const criticalIssues = testResults.filter(t => 
      t.status === 'failed' && t.priority === 'high'
    ).length + feedbackData.filter(f => f.rating <= 2).length;
  ...
  const getFeedbackTrendData = () => {
    ...
    const filteredFeedback = feedbackData.filter(f => 
      new Date(f.timestamp).getTime() > now.getTime() - timeRangeMs
    );
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{metrics.avgFeedbackRating.toFixed(1)}/5</p>
              </div>
                    ...
                    <div key={index} className="border rounded-lg p-4 space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Badge variant={issue.type === 'Test Failure' ? 'destructive' : 'secondary'}>
                            {issue.type}
                          </Badge>
                          <Badge
                        ...
                        <span className="text-xs text-gray-500">
                          {issue.timestamp ? new Date(issue.timestamp).toLocaleDateString() : 'No date'}
                        </span>
                      </div>
