...
import { Monitor, Bot, Calendar, Newspaper, Briefcase, PenTool, Gamepad2, Users, Lightbulb, Rocket, Award, Globe, Cpu } from 'lucide-react';
// Simple icon components
const DashboardIcon = () => _jsx(Monitor, { className: 'w-6 h-6' });
const BotIcon = () => _jsx(Bot, { className: 'w-6 h-6' });
const CalendarIcon = () => _jsx(Calendar, { className: 'w-6 h-6' });
const NewsIcon = () => _jsx(Newspaper, { className: 'w-6 h-6' });
const BriefcaseIcon = () => _jsx(Briefcase, { className: 'w-6 h-6' });
const PenIcon = () => _jsx(PenTool, { className: 'w-6 h-6' });
const GameIcon = () => _jsx(Gamepad2, { className: 'w-6 h-6' });
const UsersIcon = () => _jsx(Users, { className: 'w-6 h-6' });
const LightbulbIcon = () => _jsx(Lightbulb, { className: 'w-6 h-6' });
const RocketIcon = () => _jsx(Rocket, { className: 'w-6 h-6' });
const TeamIcon = () => _jsx(Users, { className: 'w-6 h-6' });
const AwardIcon = () => _jsx(Award, { className: 'w-6 h-6' });
const GlobeIcon = () => _jsx(Globe, { className: 'w-6 h-6' });
const CpuIcon = () => _jsx(Cpu, { className: 'w-6 h-6' });
...
const FeaturesSection = () => {
    ...
    const onApiChange = useCallback((api) => {
        if (!api) {
            return;
        }
        const handleSelect = () => {
    ...
    return (_jsxs("section", { id: 'features', className: 'relative py-24 bg-gradient-to-br from-gray-50 to-white overflow-hidden', children: [_jsx("div", { className: 'absolute inset-0 bg-gradient-to-r from-blue-50/30 to-purple-50/30' }), _jsx("div", { className: 'absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(120,119,198,0.05),transparent_50%)] rounded-none bg-gray-50' }), _jsxs("div", { className: 'relative container mx-auto px-4 sm:px-6 lg:px-8', children: [_jsxs("div", { className: 'text-center mb-20', children: [_jsx("h2", { className: 'text-4xl md:text-5xl lg:text-6xl font-display font-bold mb-6 bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 bg-clip-text text-transparent', children: "Transform Your GRC Workflow" }), _jsx("p", { className: 'text-muted-foreground max-w-4xl mx-auto leading-relaxed text-lg font-medium', children: "Discover AI-driven tools that revolutionize compliance management, enhance career growth, and connect you with the world's leading GRC professionals." })] }), _jsx("div", { className: 'max-w-7xl mx-auto', ref: carouselRef, children: _jsxs(Carousel, { opts: {
                                align: 'center',
                                loop: true,
                                dragFree: true,
                                skipSnaps: false
                            }, setApi: handleApiChange, className: 'w-full bg-transparent', children: [_jsx(CarouselContent, { className: '-ml-2 md:-ml-4', children: features.map((feature, index) => (_jsx(CarouselItem, { className: 'pl-2 md:pl-4 md:basis-auto px-px py-0 my-[30px]', children: _jsxs(Card, { className: cn('relative overflow-hidden transition-all duration-500 ease-out cursor-pointer group', 'border border-border/50 bg-card/80 backdrop-blur-sm shadow-lg hover:shadow-xl', activeFeature === index
                                                ? 'w-[320px] sm:w-[360px] md:w-[440px] h-[360px] md:h-[400px] z-20 scale-105 shadow-2xl border-primary/20'
                                                : 'w-[100px] sm:w-[120px] h-[360px] md:h-[400px] opacity-70 hover:opacity-90 z-10'), children: [activeFeature === index && (_jsx("div", { className: 'absolute inset-0 bg-gradient-to-br from-primary/5 to-accent/5 pointer-events-none' })), _jsx(CardContent, { className: cn('relative flex flex-col h-full p-6', activeFeature === index ? 'justify-between' : 'justify-center'), children: activeFeature === index ? (
                                                    // Enhanced expanded view
                                                    _jsxs("div", { className: 'flex flex-col h-full justify-between', children: [_jsxs("div", { className: 'space-y-6', children: [_jsx("div", { className: cn(feature.color, feature.textColor, 'rounded-2xl w-16 h-16 flex items-center justify-center shadow-lg', 'group-hover:scale-110 transition-transform duration-300'), children: _jsx(feature.icon, {}) }), _jsxs("div", { className: 'space-y-4', children: [_jsx("h3", { className: 'text-2xl font-display font-bold text-foreground leading-tight', children: feature.title }), _jsx("p", { className: 'text-muted-foreground leading-relaxed text-base', children: feature.description })] })] }), _jsxs(Button, { variant: 'ghost', className: 'w-fit text-muted-foreground hover:text-foreground gap-3 mt-6 group/btn p-0 h-auto', children: [_jsxs("span", { className: 'text-sm font-medium', children: ["Explore ", feature.title] }), _jsx("div", { className: 'w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center group-hover/btn:bg-primary group-hover/btn:text-primary-foreground transition-all duration-300', children: _jsx(Play, { className: 'h-4 w-4 group-hover/btn:translate-x-0.5 transition-transform' }) })] })] })) : (
                                                    // Enhanced collapsed view
                                                    _jsx("div", { className: 'flex items-center justify-center h-full bg-transparent', children: _jsx("div", { className: 'transform rotate-90 whitespace-nowrap', children: _jsx("span", { className: 'uppercase tracking-[0.2em] transition-colors text-indigo-950 font-semibold text-sm', children: feature.title }) }) })) })] }) }, index))) }), _jsxs("div", { className: 'flex justify-center items-center mt-12 gap-6', children: [_jsx(Button, { onClick: () => api?.scrollPrev(), variant: 'outline', size: 'icon-lg', className: 'border-2 hover:scale-110 transition-all duration-300 shadow-md hover:shadow-lg', "aria-label": 'Navigate to previous feature', children: _jsx(ChevronLeft, { className: 'h-5 w-5' }) }), _jsx("div", { className: 'flex items-center gap-2', children: features.map((_, index) => (_jsx(Button, { variant: 'ghost', size: 'icon-sm', onClick: () => api?.scrollTo(index), className: cn('w-2 h-2 rounded-full p-0 transition-all duration-300', activeFeature === index
                                                    ? 'bg-primary w-8 h-2'
                                                    : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'), "aria-label": `Go to feature ${index + 1}`, "aria-current": activeFeature === index ? 'true' : 'false' }, index))) }), _jsx(Button, { onClick: () => api?.scrollNext(), variant: 'outline', size: 'icon-lg', className: 'border-2 hover:scale-110 transition-all duration-300 shadow-md hover:shadow-lg', "aria-label": 'Navigate to next feature', children: _jsx(ChevronRight, { className: 'h-5 w-5' }) })] })] }) })] })] }));
};
