...
export function ListSkeleton({ items = 3 }) {
  return _jsx('div', {
    className: 'space-y-3',
    children: Array.from({ length: items }).map((_, i) =>
      _jsxs(
        'div',
        {
          className: 'flex items-center space-x-4',
          children: [
            _jsx(Skeleton, { className: 'h-12 w-12 rounded-full' }),
            _jsxs('div', {
              className: 'space-y-2',
              children: [
                _jsx(Skeleton, { className: 'h-4 w-[200px]' }),
                _jsx(Skeleton, { className: 'h-4 w-[160px]' })
              ]
            })
          ]
        },
...
export function TableSkeleton({ rows = 5, columns = 4 }) {
  return _jsx('div', {
    ...
    children: _jsx('div', {
      ...
      children: Array.from({ length: rows * columns }).map((_, i) =>
