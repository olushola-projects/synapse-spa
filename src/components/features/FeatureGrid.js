...
export const FeatureGrid = ({ animate, onFeatureClick }) => {
  return _jsx('div', {
    className: 'max-w-xl',
    children: _jsx('div', {
      className: 'grid grid-cols-3 gap-2 sm:gap-3 transition-all duration-700 ease-out',
      children: featureIcons.map((feature, index) =>
        _jsxs(
          motion.div,
          {
            className:
              'flex flex-col items-center justify-center aspect-square p-2 sm:p-3 rounded-lg bg-white hover:bg-white/95 shadow-md hover:shadow-lg border border-gray-200 cursor-pointer hover:-translate-y-1 transition-all duration-300 hover:border-indigo-500',
            ...
            children: [
              _jsx('div', {
                className:
                  'w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gray-50 flex items-center justify-center mb-1 sm:mb-2',
                children: feature.icon
              }),
              _jsx('h3', {
                className: 'text-xs sm:text-sm font-semibold mb-0.5 text-center line-clamp-1',
                children: feature.title
              }),
              _jsx('p', {
                className: 'text-[10px] sm:text-xs text-gray-600 text-center',
