...
function SuccessRateDisplay({ rate, color }) {
  return _jsxs('div', {
    ...
    children: [
      ...
      _jsx(Progress, { value: rate, className: 'w-full h-2 bg-gray-200 rounded-full' })
    ]
  });
}
export function ApiSuccessCard({ successRate, isLoading = false }) {
  ...
  return _jsxs(Card, {
    children: [
      _jsx(CardHeader, { children: _jsx(CardTitle, { children: 'API Success Rate' }) }),
      _jsx(CardContent, {
        className: 'flex flex-col items-center justify-center space-y-4',
        children: _jsx(SuccessRateDisplay, { rate: successRate, color: statusColor })
      })
    ]
  });
}
