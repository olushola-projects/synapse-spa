...
const FeaturesSection = () => {
  ...
  return <section id='features' className='relative py-24 bg-gradient-to-br from-gray-50 to-white overflow-hidden'>
      {/* Subtle background pattern */}
      <div className='absolute inset-0 bg-gradient-to-r from-blue-50/30 to-purple-50/30' />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(120,119,198,0.05),transparent_50%)] rounded-none bg-gray-50" />
      ...
      <div className='relative container mx-auto px-4 sm:px-6 lg:px-8'>
        ...
        <div className='text-center mb-20'>
          ...
          <p className='text-muted-foreground max-w-4xl mx-auto leading-relaxed text-lg font-medium'>
            Discover AI-driven tools that revolutionize compliance management, enhance career
            growth, and connect you with the world's leading GRC professionals.
          </p>
        </div>
        ...
        <div className='max-w-7xl mx-auto' ref={carouselRef}>
          ...
          dragFree: true,
          skipSnaps: false
        }} setApi={handleApiChange} className="w-full bg-transparent">
            <CarouselContent className='-ml-2 md:-ml-4'>
              {features.map((feature, index) => <CarouselItem key={index} className='pl-2 md:pl-4 md:basis-auto px-px py-0 my-[30px]'>
                  <Card className={cn('relative overflow-hidden transition-all duration-500 ease-out cursor-pointer group', 'border border-border/50 bg-card/80 backdrop-blur-sm shadow-lg hover:shadow-xl', activeFeature === index ? 'w-[320px] sm:w-[360px] md:w-[440px] h-[360px] md:h-[400px] z-20 scale-105 shadow-2xl border-primary/20' : 'w-[100px] sm:w-[120px] h-[360px] md:h-[400px] opacity-70 hover:opacity-90 z-10')}>
                    {/* Gradient overlay for active card */}
                    {activeFeature === index && <div className='absolute inset-0 bg-gradient-to-br from-primary/5 to-accent/5 pointer-events-none' />}

                    <CardContent className={cn('relative flex flex-col h-full p-6', activeFeature === index ? 'justify-between' : 'justify-center')}>
                      {activeFeature === index ?
                  // Enhanced expanded view
                  <div className='flex flex-col h-full justify-between'>
                          <div className='space-y-6'>
                            {/* Icon with enhanced styling */}
                            <div className={cn(feature.color, feature.textColor, 'rounded-2xl w-16 h-16 flex items-center justify-center shadow-lg', 'group-hover:scale-110 transition-transform duration-300')}>
                              <feature.icon />
                            </div>
                          ...
                          {/* Enhanced CTA */}
                          <Button variant='ghost' className='w-fit text-muted-foreground hover:text-foreground gap-3 mt-6 group/btn p-0 h-auto'>
