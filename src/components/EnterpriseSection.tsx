...
const agentData = [{
  ...
  valueAdd: ['Automates document verification and metadata analysis.', 'Flags risk patterns and compiles enhanced due diligence reports.', 'Functions as a Tier-2 AML analyst within your case management tool.'],
  outcomes: ['Accelerated onboarding processes.', 'Reduction in false positives.', 'Enhanced compliance accuracy.'],
  metrics: [{
  ...
  valueAdd: ['Filters false positives, flags exceptions', 'Synthesizes regulatory rules', 'Automates routine exception handling'],
  outcomes: ['Faster exception triage', 'Improved audit preparedness', 'Reduced operational risk'],
  metrics: [{
  ...
  valueAdd: ['Reviews content for compliance', 'Highlights jurisdictional risks', 'Suggests compliant alternatives'],
  outcomes: ['Reduced regulatory risk', 'Improved cross-border alignment', 'Faster promotion approvals'],
  metrics: [{
  ...
  valueAdd: ['Interprets ESG regulations', 'Flags disclosure gaps', 'Monitors reporting requirements'],
  ...
  valueAdd: ['Contextualizes complaints', 'Suggests case categorization', 'Identifies emerging patterns'],
  outcomes: ['More accurate resolution outcomes', 'Enhanced regulatory reporting', 'Proactive risk identification'],
  metrics: [{
  ...
  outcomes: ['Faster report creation', 'Improved decision-making', 'Enhanced governance insights'],
  metrics: [{
...
const EnterpriseSection = () => {
  ...
  return <section className='bg-[#F9FAFB] py-16 md:py-[90px]'>
        ...
        <div className='grid md:grid-cols-2 gap-12 items-start'>
          ...
          once: true
            ...
            <motion.div variants={itemVariants}>
              <p className="text-[#4B4B4B] leading-relaxed font-normal text-base">
                Seamlessly integrate AI agents to streamline compliance, enhance decisions, and
                reduce manual work across your existing GRC ecosystem.
              </p>
              <p className="mt-4 text-[#4B4B4B] leading-relaxed font-normal text-base">
                <a href='#' onClick={e => {
                e.preventDefault();
                openFormDialog('Learn about Professional Services');
              }} className='text-[#7A73FF] hover:text-[#6366F1] underline-offset-4 hover:underline'>
            ...
            <motion.div variants={itemVariants} className='pt-6 mt-8 border-t border-gray-200'>
              <div className='grid grid-cols-3 gap-6'>
                <div>
                  <h4 className="text-gray-900 mb-1 text-base font-semibold">Status</h4>
                  <p className="text-xs text-slate-900 font-normal">Early Access Program Open</p>
                </div>

                <div>
                  <h4 className="text-gray-900 mb-1 text-base font-semibold">Regions</h4>
                  <div className='flex flex-col gap-1'>
                    <p className="text-xs font-normal text-slate-900">Global Regulatory Coverage</p>
                    <p className="font-normal text-xs text-slate-900">Continuously Expanding Jurisdictions</p>
                  </div>
                </div>

                <div>
                  <h4 className="text-gray-900 mb-1 text-base font-semibold">Powered by Innovation</h4>
                  <div className='flex items-center gap-4 mt-2'>
                    <div className='flex items-center gap-1 text-gray-600'>
                      
                      <span className="text-xs font-normal text-slate-900">AML Agent</span>
                    </div>
                    <div className='flex items-center gap-1 text-gray-600'>
                      
                      <span className="text-xs font-normal">ESG Agent</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          once: true
            ...
            align: 'start',
            loop: true
          }} className='w-full' ref={emblaRef}>
              <CarouselContent>
                {agentData.map((agent, index) => <CarouselItem key={index} className='md:basis-[100%] lg:basis-[100%]'>
                        ...
                        <CardContent className='p-6 relative z-10 h-full flex flex-col'>
                          ...
                          <div className='mb-4'>
                            ...
                            <div className='flex flex-wrap gap-2'>
                              {agent.integratedTools.map((tool, i) => <Badge key={i} variant='outline' className='bg-blue-50 text-blue-700'>
                                  {tool}
                                </Badge>)}
                            </div>
                          </div>
                          ...
                          <div className='mb-4'>
                            ...
                            <ul className='space-y-2'>
                              {agent.valueAdd.map((value, i) => <li key={i} className='flex items-start gap-2 text-sm'>
                          ...
                          <div className='mb-4 p-3 bg-gray-50 rounded-lg border border-gray-100'>
                            ...
                            <ul className='space-y-1'>
                              {agent.outcomes.map((outcome, i) => <li key={i} className='text-sm text-gray-700'>
                                  • {outcome}
                                </li>)}
                            </ul>
                          </div>
                          ...
                          <div className='mt-auto pt-3'>
                            ...
                            {agent.metrics.map((metric, i) => <div key={i} className='mb-2 last:mb-0'>
                          ...
                          <div className='absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity'>
                            <Button variant='ghost' size='sm' className='text-[#7A73FF] hover:bg-[#7A73FF]/10 p-0'>
      ...
      <ExternalFormDialog open={showFormDialog} onOpenChange={setShowFormDialog} title={dialogTitle} />
    </section>;
};
