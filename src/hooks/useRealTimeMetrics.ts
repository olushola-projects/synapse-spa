    ...
    try {
      ...
      if (data && typeof data === 'object') {
        const validatedMetrics: RealTimeMetrics = {
          complianceScore: Math.max(
            0,
            Math.min(100, data.complianceScore || initialMetrics.complianceScore)
          ),
          riskReduction: Math.max(
            0,
            Math.min(100, data.riskReduction || initialMetrics.riskReduction)
          ),
      ...
      setMetrics(prev => ({
        ...
        processingSpeed: Math.max(
          2.5,
          Math.min(4.0, prev.processingSpeed + (Math.random() - 0.5) * 0.3)
        ),
        activeUsers: Math.max(
          450,
          Math.min(550, prev.activeUsers + Math.floor((Math.random() - 0.5) * 20))
        ),
        lastUpdated: new Date().toISOString()
      }));
    } finally {
