...
import { NexusAgentClient, NexusClassificationRequest, NexusClassificationResponse } from '@/services/nexusAgentClient';
...
export const useSFDRClassification = (): SFDRClassificationHookResult => {
  ...
  const classify = async (productData: NexusClassificationRequest): Promise<NexusClassificationResponse> => {
    setLoading(true);
    setError(null);
    
    try {
      // Get API key from Supabase secrets (this would be set via environment or edge function)
      const client = new NexusAgentClient({
        apiKey: process.env.NEXUS_API_KEY // Will be set via Supabase secrets
      });
