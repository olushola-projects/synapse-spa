...
export function useDocumentAnalysis(): UseDocumentAnalysisReturn {
  ...
  const searchAnalyses = useCallback(async (query: DocumentAnalysisSearchQuery = {}) => {
    setLoading(true);
    setError(null);
    
    try {
      const result: DocumentAnalysisSearchResult = await DocumentAnalysisAPI.searchDocumentAnalyses(query);
  ...
  const loadRecentAnalyses = useCallback(async (limit = 10) => {
    setLoading(true);
    setError(null);
  ...
  const loadThreadAnalyses = useCallback(async (threadId: string, limit = 20) => {
    setLoading(true);
    setError(null);
  ...
  const getAnalysis = useCallback(async (id: string): Promise<StoredDocumentAnalysis | null> => {
    try {
