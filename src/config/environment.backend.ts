...
export function getBackendEnvironmentConfig(): BackendEnvironmentConfig {
  ...
  return {
    ...
    // Feature Flags
    ENABLE_DEBUG_MODE: parseBoolean(process.env.ENABLE_DEBUG_MODE, process.env.NODE_ENV === 'development'),
    
    // Security
    CORS_ORIGINS: parseArray(process.env.CORS_ORIGINS, ['http://localhost:3000', 'http://localhost:8080'])
  };
}
