...
const SFDRNavigator: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      if (userMessage.toLowerCase().includes('article 8')) {
        response = 'Article 8 funds promote environmental or social characteristics under SFDR. Key requirements include pre-contractual disclosures (Article 10), website disclosures (Article 11), and periodic reporting (Article 12). These funds must disclose how environmental or social characteristics are met and provide information on sustainability indicators.';
        citations = ['SFDR Article 8', 'SFDR Article 10', 'SFDR Article 11', 'SFDR Article 12', 'Commission Regulation (EU) 2022/1288'];
        confidence = 95;
        classification = 'Article 8';
      } else if (userMessage.toLowerCase().includes('article 9')) {
        response = 'Article 9 funds have sustainable investment as their objective under SFDR. They must demonstrate measurable positive environmental or social impact and apply the "do no significant harm" principle. Enhanced disclosure requirements include taxonomy alignment reporting and principal adverse impact considerations.';
        citations = ['SFDR Article 9', 'SFDR Article 2(17)', 'EU Taxonomy Regulation (EU) 2020/852', 'Commission Regulation (EU) 2022/1288 Article 5'];
        confidence = 98;
        classification = 'Article 9';
      } else if (userMessage.toLowerCase().includes('pai') || userMessage.toLowerCase().includes('principal adverse')) {
        response = 'Principal Adverse Impacts (PAI) are mandatory considerations for large financial market participants (>€500M AUM). PAI indicators cover environmental (GHG emissions, biodiversity, water, waste) and social/governance aspects. Disclosure required in pre-contractual documents and periodic reports.';
        citations = ['SFDR Article 4', 'Commission Regulation (EU) 2022/1288 Annex I', 'SFDR Article 7', 'RTS Article 5'];
        confidence = 92;
      } else {
  ...
  const handleClassification = useCallback(async () => {
    ...
    try {
      ...
      const stages = ['Analyzing fund documentation...', 'Applying SFDR criteria...', 'Generating regulatory citations...', 'Calculating compliance score...'];
      ...
      setAnalyticsData(prev => ({
        ...
        [mockResult.classification.toLowerCase().replace(' ', '') as keyof typeof prev]: 
          (prev[mockResult.classification.toLowerCase().replace(' ', '') as keyof typeof prev] as number) + 1,
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const mockAnalysis: DocumentAnalysis = {
          ...
          entities: ['ESG Integration', 'Sustainable Investment', 'Climate Risk', 'PAI Indicators'],
          sentiment: 'positive',
  ...
  const handleExport = useCallback((format: 'pdf' | 'excel' | 'json') => {
    const exportData = {
      ...
      citations: Array.from(new Set(
        messages.flatMap(m => m.metadata?.citations || [])
      ))
    };
    ...
    addMessage('system', `Analysis exported as ${format.toUpperCase()} with complete regulatory citations.`);
      ...
      <div className="w-full max-w-7xl mx-auto p-6 space-y-6">
        ...
        >
          <div className="flex items-center justify-center space-x-3">
            ...
            <Badge variant="secondary" className="px-3 py-1 bg-gradient-to-r from-green-100 to-emerald-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
            ...
            <div className="text-center p-3 bg-white/50 rounded-lg border">
              <div className="text-2xl font-bold text-green-600">{analyticsData.complianceScore.toFixed(1)}%</div>
              ...
              >
                ...
                <TabsContent value="chat" className="space-y-4 mt-0">
                      ...
                      >
                        <div className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-gray-600">Analyzing with regulatory citations...</span>
                          </div>
                        </div>
                      </motion.div>
                    )}
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      All responses include mandatory regulatory citations per SFDR compliance requirements. 
                      Chat history is automatically saved for audit trails.
                    </AlertDescription>
                  </Alert>
                </TabsContent>
                            ...
                            <div>
                              ...
                              <ul className="text-sm space-y-2">
                                {classificationResult.recommendations.map((rec, index) => (
                                  <li key={index} className="flex items-start bg-white p-2 rounded border">
                              ...
                              <div>
                                ...
                                <ul className="text-sm space-y-2">
                                  {classificationResult.issues.map((issue, index) => (
                                    <li key={index} className="flex items-start bg-yellow-50 p-2 rounded border border-yellow-200">
                ...
                <TabsContent value="documents" className="space-y-4 mt-0">
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50/50">
                    ...
                    <p className="text-muted-foreground mb-4">
                      Upload fund prospectuses, ESG reports, or compliance documents for AI analysis with regulatory citations
                    </p>
                    <input
                      ref={fileInputRef}
                      type="file"
                          ...
                          <Card className="hover:shadow-md transition-shadow">
                            <CardContent className="p-4">
                              ...
                              <div className="space-y-2">
                                ...
                                {doc.complianceIssues.length > 0 && (
                                  <div className="text-sm">
                                    <span className="font-medium text-yellow-600">Compliance Issues:</span>
                                    <ul className="mt-1 space-y-1">
                                      {doc.complianceIssues.map((issue, i) => (
                                        <li key={i} className="text-xs text-yellow-700 bg-yellow-50 p-1 rounded">
                                          • {issue}
                                        </li>
                                      ))}
                                    </ul>
                ...
                <TabsContent value="analytics" className="space-y-6 mt-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <Card className="text-center p-4 bg-gradient-to-br from-blue-50 to-blue-100">
                      <CardContent className="p-0">
                        <div className="text-3xl font-bold text-blue-600">{analyticsData.totalFunds}</div>
                    ...
                    <Card className="text-center p-4 bg-gradient-to-br from-green-50 to-green-100">
                      <CardContent className="p-0">
                        <div className="text-3xl font-bold text-green-600">{analyticsData.article8}</div>
                    ...
                    <Card className="text-center p-4 bg-gradient-to-br from-purple-50 to-purple-100">
                      <CardContent className="p-0">
                        <div className="text-3xl font-bold text-purple-600">{analyticsData.article9}</div>
                    ...
                    <Card className="text-center p-4 bg-gradient-to-br from-orange-50 to-orange-100">
                      <CardContent className="p-0">
                        <div className="text-3xl font-bold text-orange-600">{analyticsData.article6}</div>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      Analytics updated in real-time. All classifications include mandatory regulatory citations 
                      for comprehensive audit trails. Last update: {analyticsData.lastUpdate.toLocaleString()}
                    </AlertDescription>
                  </Alert>
                </TabsContent>
                  ...
                  <div className="text-center space-y-6">
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Export comprehensive SFDR analysis with regulatory citations in multiple formats
                      </p>
                    </div>
                    ...
                    {(classificationResult || uploadedDocuments.length > 0 || messages.length > 1) && (
                      <Alert className="bg-green-50 border-green-200">
                        ...
                        <AlertDescription className="text-green-700">
                          ...
                          {messages.length} chat messages with {' '}
                          {Array.from(new Set(messages.flatMap(m => m.metadata?.citations || []))).length} unique regulatory citations
                        </AlertDescription>
                      </Alert>
                    )}
                  </div>
