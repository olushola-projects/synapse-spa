...
import { supabase } from '@/lib/mock-auth';
const NexusAgent = () => {
  ...
  // Filter actions for suggestions (all except upload-document and generate-report)
  const suggestionActions = useMemo(
    () =>
      quickActions
        .filter(action => action.type !== 'upload-document' && action.type !== 'generate-report')
