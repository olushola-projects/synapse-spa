...
export class PerformanceMonitor {
     ...
     */
    trackClassification(startTime, endTime, request, response, success, error) {
        const event = {
            type: 'classification',
     ...
     */
    trackHealthCheck(duration, success, status) {
        const event = {
            type: 'health_check',
            timestamp: new Date().toISOString(),
            duration,
    ...
    getHealthStatus() {
        ...
        const lastHealthCheck = healthChecks.length > 0 ? healthChecks[healthChecks.length - 1]?.timestamp : undefined;
        if (!lastHealthCheck) {
...
export const withPerformanceMonitoring = async (operation, context) => {
    ...
    try {
        ...
        const endTime = performance.now();
        performanceMonitor.trackClassification(startTime, endTime, context.request, null, false, error instanceof Error ? error.message : 'Unknown error');
        throw error;
    }
};
