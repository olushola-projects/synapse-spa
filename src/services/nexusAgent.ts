...
class NexusAgentService {
   ...
   */
  async validateClassification(request: SFDRClassificationRequest): Promise<NexusValidationResponse> {
    try {
   ...
   */
  private generateEnhancedMockValidation(request: SFDRClassificationRequest): NexusValidationResponse {
    ...
    if (targetArticle === 'Article8') {
      if (!fundProfile.sustainabilityCharacteristics || fundProfile.sustainabilityCharacteristics.length === 0) {
        issues.push({
          id: 'SFDR_ART8_001',
    ...
    const validationDetails: ValidationDetails = {
      ...
      dataQuality: request.paiIndicators?.dataQuality?.coveragePercentage ? 
        request.paiIndicators.dataQuality.coveragePercentage > 80 : false,
  ...
  private determineRecommendedArticle(request: SFDRClassificationRequest): 'Article6' | 'Article8' | 'Article9' {
  ...
  private calculateConfidence(request: SFDRClassificationRequest, issues: ValidationIssue[]): number {
    let confidence = 0.95;
    
    // Reduce confidence based on issues
    issues.forEach(issue => {
      if (issue.severity === 'error') confidence -= 0.2;
      if (issue.severity === 'warning') confidence -= 0.1;
    });
    
    // Increase confidence if comprehensive data provided
    if (request.paiIndicators) confidence += 0.05;
    if (request.taxonomyAlignment) confidence += 0.05;
  ...
  private generateAlternatives(request: SFDRClassificationRequest, currentArticle: string) {
    ...
    if (currentArticle !== 'Article6') {
      alternatives.push({
        ...
        confidence: 0.3,
        conditions: ['Remove sustainability characteristics', 'Focus on traditional investment approach']
      });
    }
