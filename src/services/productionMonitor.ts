...
class ProductionMonitoringService {
   ...
   */
  recordMetric(serviceName: string, responseTime: number, success: boolean, error?: string): void {
   ...
   */
  private checkAlertConditions(serviceName: string, metric: ServiceMetrics): void {
    const alerts: Alert[] = [];
   ...
   */
  private getRecentMetrics(serviceName: string, hours: number): ServiceMetrics[] {
  ...
  getServiceHealth(serviceName: string): {
    ...
    const lastError = recentMetrics
      ...
      .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())[0]?.lastError;
  ...
  getPerformanceTrends(serviceName: string, hours: number = 24): {
    ...
    const hourlyGroups = new Map<string, ServiceMetrics[]>();
    ...
    Array.from(hourlyGroups.entries())
      ...
      .forEach(([hour, hourMetrics]) => {
        ...
        const totalRequests = hourMetrics.reduce((sum, m) => sum + m.errorCount + m.successCount, 0);
        const totalErrors = hourMetrics.reduce((sum, m) => sum + m.errorCount, 0);
        const errorRate = totalRequests > 0 ? totalErrors / totalRequests : 0;
        errorRates.push(Math.round(errorRate * 100));
        
        const avgAvailability = hourMetrics.reduce((sum, m) => sum + m.availability, 0) / hourMetrics.length;
