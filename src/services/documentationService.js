...
- **Status**: Approved`,
    ...
    async checkDocumentReviews() {
        try {
            const now = new Date();
            const documentsNeedingReview = Array.from(this.documents.values()).filter(doc => doc.nextReviewDate && doc.nextReviewDate <= now);
    ...
    async searchDocuments(search) {
        ...
        if (search.filters.dateRange) {
            documents = documents.filter(d => d.updatedAt >= search.filters.dateRange.start &&
                d.updatedAt <= search.filters.dateRange.end);
        }
        // Apply search query
        if (search.query) {
            const query = search.query.toLowerCase();
            documents = documents.filter(d => d.title.toLowerCase().includes(query) ||
