...
class AuthService {
    ...
    async authenticateUser(email, password, ipAddress, userAgent) {
        try {
            ...
            if (error || !user) {
                await this.handleFailedLogin(email, ipAddress, userAgent, error?.message || 'Authentication failed');
                return { success: false, error: error?.message || 'Authentication failed' };
            }
            // Check if user account is active - use profiles table instead of user_profiles
    ...
    async validateToken(token, ipAddress) {
        try {
            ...
            // Get user data - use profiles table instead of user_profiles
     ...
     */
    async logout(sessionId, ipAddress) {
        try {
    ...
    async invalidateSession(sessionId) {
        ...
        // Update database - use user_sessions table instead of sessions
        await supabase
            .from('user_sessions')
     ...
     */
    async handleFailedLogin(email, ipAddress, userAgent, reason) {
    ...
    async logSecurityEvent(event) {
        try {
            // Log to database - use security_events table
