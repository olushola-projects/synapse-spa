...
export class LLMConfigurationService {
    ...
    getDefaultStrategy() {
        ...
        if (!defaultStrategy) {
            throw new Error(`Default strategy '${this.configuration.defaultStrategy}' not found or disabled`);
        }
    ...
    getBestModelForTask(task, strategyId) {
        ...
        const capableModels = availableModels.filter(m => m.capabilities.some(cap => cap.toLowerCase().includes(task.toLowerCase())));
    ...
    getConfigurationSummary() {
        ...
        return {
            ...
            enabledModels: this.configuration.strategies.reduce((sum, s) => sum + s.models.filter(m => m.enabled).length, 0),
