...
export interface ThreatIndicator {
  type: 'ip' | 'user_agent' | 'behavior_pattern' | 'geolocation' | 'domain';
  value: string;
...
class SecurityMonitoringService {
  ...
  private wazuhEndpoint?: string;
  private falcoEndpoint?: string;
   ...
   */
  private async testWazuhConnection(): Promise<void> {
    try {
   ...
   */
  private async testFalcoConnection(): Promise<void> {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 5000);
