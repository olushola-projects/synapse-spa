 ...
 */
router.get('/compliance/rules', authenticateJWT, requireRole(['admin', 'compliance']), async (_req, res) => {
    try {
 ...
 */
router.get('/compliance/checks', authenticateJWT, requireRole(['admin', 'compliance']), async (_req, res) => {
    try {
 ...
 */
router.post('/compliance/execute-check/:ruleId', authenticateJWT, requireRole(['admin']), async (req, res) => {
    try {
 ...
 */
router.post('/compliance/reports', authenticateJWT, requireRole(['admin', 'compliance']), async (req, res) => {
    try {
        const { startDate, endDate } = req.body;
        // const generatedBy = (req as any).user?.id || 'system';
        const report = await complianceAutomationService.generateComplianceReport({ start: new Date(startDate), end: new Date(endDate) });
 ...
 */
router.get('/performance/metrics', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
 ...
 */
router.post('/performance/metrics', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
 ...
 */
router.get('/performance/alerts', authenticateJWT, requireRole(['admin', 'devops']), async (_req, res) => {
    try {
 ...
 */
router.post('/performance/alerts/:alertId/acknowledge', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
 ...
 */
router.post('/performance/thresholds', authenticateJWT, requireRole(['admin']), async (req, res) => {
    try {
 ...
 */
router.get('/performance/optimizations', authenticateJWT, requireRole(['admin', 'devops']), async (_req, res) => {
    try {
 ...
 */
router.post('/performance/optimizations/:optimizationId/implement', authenticateJWT, requireRole(['admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: 'Failed to implement performance optimization' });
    }
});
/**
 ...
 */
router.post('/performance/reports', authenticateJWT, requireRole(['admin', 'devops']), async (_req, res) => {
    try {
 ...
 */
router.get('/errors/circuit-breakers', authenticateJWT, requireRole(['admin', 'devops']), async (_req, res) => {
    try {
 ...
 */
router.get('/errors/circuit-breakers/:name', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
 ...
 */
router.post('/errors/circuit-breakers/:name/reset', authenticateJWT, requireRole(['admin']), async (req, res) => {
    try {
        const { name } = req.params;
        if (!name) {
            res.status(400).json({ success: false, error: 'Circuit breaker name is required' });
 ...
 * Get error events
 */
router.get('/errors/events', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
 ...
 */
router.post('/errors/events/:errorId/handle', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
 ...
 */
router.get('/errors/degradation-strategies', authenticateJWT, requireRole(['admin', 'devops']), async (_req, res) => {
    try {
 ...
 */
router.put('/errors/degradation-strategies/:strategyId', authenticateJWT, requireRole(['admin']), async (req, res) => {
    try {
 ...
 */
router.post('/errors/reports', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
        const { startDate, endDate } = req.body;
 ...
 */
router.post('/execute-with-circuit-breaker', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
        const { name, config } = req.body;
        // This is a simplified example - in practice, you'd have predefined operations
        const result = await errorHandlingService.executeWithCircuitBreaker(name, async () => {
 ...
 */
router.post('/execute-with-retry', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
        const { name, config } = req.body;
        const result = await errorHandlingService.executeWithRetry(name, async () => {
            // Simulate the operation
            await new Promise(resolve => setTimeout(resolve, 1000));
            return { message: 'Operation completed successfully' };
        }, config);
 ...
 */
router.post('/execute-with-timeout', authenticateJWT, requireRole(['admin', 'devops']), async (req, res) => {
    try {
