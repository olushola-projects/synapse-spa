...
test.describe('Performance & Load Tests', () => {
    ...
    test.describe('API Performance', () => {
        test('should complete SFDR classification within acceptable time', async ({ page }) => {
            ...
            await page.getByTestId('fund-description-input').fill('A fund focused on sustainable investments with ESG integration');
            const startTime = Date.now();
    ...
    test.describe('Concurrent User Load Testing', () => {
        ...
        test('should handle concurrent SFDR classifications', async ({ browser }) => {
            ...
            await Promise.all(pages.map(page => expect(page.getByTestId('classification-result')).toBeVisible({ timeout: 30000 })));
            const classificationTime = Date.now() - startTime;
