...
test.describe('SFDR Navigator Core Functionality', () => {
  ...
  test('should test responsive design across different viewports', async ({
    sfdrNavigatorPage,
    responsiveTest
  }) => {
  ...
  test('should test tab navigation and content switching', async ({ sfdrNavigatorPage }) => {
    ...
    for (const tab of tabs) {
      ...
      await expect(
        sfdrNavigatorPage.locator(
          `[data-testid="tab-content-${tab.toLowerCase().replace(' ', '-')}"]`
        )
      ).toBeVisible();
    }
  });
  ...
  test('should test form submission and validation', async ({ sfdrNavigatorPage }) => {
    ...
    await sfdrNavigatorPage.fill(
      '[data-testid="fund-description-input"]',
      'Valid fund description with sufficient detail'
    );
  ...
  test('should test API integration and error handling', async ({
    sfdrNavigatorPage,
    mockAPIResponses
  }) => {
  ...
  test('should test accessibility compliance with axe-core', async ({
    sfdrNavigatorPage,
    accessibilityAudit
  }) => {
  ...
  test('should test data persistence and state management', async ({ sfdrNavigatorPage }) => {
    ...
    await expect(sfdrNavigatorPage.locator('[data-testid="fund-name-input"]')).toHaveValue(
      'Persistent Fund'
    );
  });
});
