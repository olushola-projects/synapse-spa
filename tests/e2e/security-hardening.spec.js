...
test.describe('Security Hardening Tests', () => {
        ...
        test('should prevent CSRF attacks', async ({ page }) => {
            ...
            for (const form of forms) {
                ...
                if (action && action.includes('/api/')) {
                    const tokenInput = await form.locator('input[name*="token"], input[name*="csrf"]').count();
    ...
    test.describe('Session Management', () => {
        ...
        test('should handle logout securely', async ({ page }) => {
            ...
            const cookies = await page.context().cookies();
            const sessionCookies = cookies.filter(cookie => cookie.name.includes('session') || cookie.name.includes('auth'));
