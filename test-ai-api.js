...
async function runAllTests() {
  ...
  console.log(
    `Qwen API Key: ${metrics?.api_keys_configured?.qwen ? '✅ Configured' : '❌ Missing'}`
  );
  console.log(
    `OpenAI API Key: ${metrics?.api_keys_configured?.openai ? '✅ Configured' : '❌ Missing'}`
  );
  console.log(`Classification: ${classification?.classification || 'Failed'}`);
  console.log(`Confidence: ${classification?.confidence || 'N/A'}`);
  console.log(
    `AI Features: ${metrics?.features?.includes('AI-powered classification') ? '✅ Active' : '❌ Inactive'}`
  );
}
