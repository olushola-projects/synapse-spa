...
function generateTestReport() {
  ...
  const summary = {
    ...
    passRate: (
      (CONFIG.testResults.filter(r => r.passed).length / CONFIG.testResults.length) *
      100
    ).toFixed(2),
    averageDuration: (
      CONFIG.testResults.reduce((sum, r) => sum + r.duration, 0) / CONFIG.testResults.length
    ).toFixed(2),
  ...
  console.log(
    `📈 Summary: ${summary.passed}/${summary.totalTests} tests passed (${summary.passRate}%)`
  );
}
  ...
  static async runResponsiveTests() {
    ...
    for (const viewport of CONFIG.viewports) {
      ...
      try {
        ...
        logTestResult(
          `RD-${viewport.width}`,
          `Responsive Design - ${viewport.width}x${viewport.height}`,
          false,
          duration,
          error.message
        );
