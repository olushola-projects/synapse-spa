...
function analyzeReputationalRisk(assessment: any, issues: any[]) {
  ...
  if (['Article8', 'Article9'].includes(assessment.target_article)) {
    const sustainabilityIssues = issues.filter(
      issue =>
  ...
  return {
    ...
    description:
      'Risk of reputational damage due to compliance failures or greenwashing accusations'
  };
}
...
function getSpecificMitigation(category: string): string {
  const mitigations: { [key: string]: string } = {
    'Regulatory Compliance':
      'Review and update compliance procedures, engage regulatory counsel, implement immediate corrective actions',
    Operational:
      'Strengthen operational controls, improve data collection processes, enhance reporting procedures',
    Reputational:
      'Develop crisis communication plan, engage with stakeholders, improve transparency in sustainability reporting',
    Financial:
      'Assess financial impact, establish remediation budget, consider compliance insurance',
    'Data Quality':
      'Implement data validation controls, establish data quality metrics, improve data governance'
  };
...
function getProcessImprovement(category: string): string {
  const improvements: { [key: string]: string } = {
    'Regulatory Compliance':
      'Establish compliance calendar, implement regular regulatory updates review, create compliance dashboard',
    Operational:
      'Automate manual processes, implement quality controls, establish backup procedures',
    Reputational:
      'Create sustainability reporting standards, implement greenwashing prevention controls',
    Financial:
      'Establish cost-benefit analysis framework, implement financial impact assessment procedures',
    'Data Quality':
      'Create data quality framework, implement automated data validation, establish data lineage tracking'
  };
...
function getMitigationResources(category: string): string[] {
  ...
  return (
    resources[category] || ['Expert consultation', 'Process documentation', 'Training materials']
  );
}
