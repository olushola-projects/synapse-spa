...
```tsx
// Required fixes
<Button aria-label='Classify fund for SFDR compliance' onKeyDown={handleKeyNavigation} tabIndex={0}>
  Classify Fund
</Button>
```
...
const validatePAIIndicators = (indicators: string[]) => {
  ...
  return mandatoryIndicators.every(indicator => indicators.includes(indicator));
};
```
